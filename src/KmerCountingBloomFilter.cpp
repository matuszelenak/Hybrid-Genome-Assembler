...
KmerCount KMER_COUNT_MAX = UINT8_MAX;

uint16_t CACHE_LINE_SIZE = 512;
...
uint32_t MAX_BIN_COUNT = UINT32_MAX;
...
void KmerCountingBloomFilter::add(Kmer kmer) {
    ...
    KmerCount *start_cell = data + (*((uint32_t *) hash_output) % bins) * BIN_SIZE;
    uint8_t *remaining_hash_ptr = (uint8_t *) hash_output + sizeof(uint32_t);
...
bool KmerCountingBloomFilter::contains(Kmer kmer) {
    void *hash_output = malloc(16);
    MurmurHash3_x64_128(&kmer, sizeof(Kmer), 0, hash_output);
    uint32_t bin_index = *((uint32_t *) hash_output) % bins;
    bool contains = true;
    for (int i = 0; i < hash_count; i++) {
        uint8_t inner_index = *((uint8_t *) hash_output + i + sizeof(uint32_t)) & INNER_INDEX_MASK;
...
KmerCount KmerCountingBloomFilter::get_count(Kmer kmer) {
    ...
    for (int i = 0; i < hash_count; i++) {
        uint8_t inner_index = *((uint8_t *) hash_output + i + sizeof(uint32_t)) & INNER_INDEX_MASK;
        count = std::min(data[bin_index * BIN_SIZE + inner_index], count);
    }
    free(hash_output);
    return count;
}
...
KmerCountingBloomFilter::KmerCountingBloomFilter(uint64_t expected_items, double fp_prob) {
    int bits_for_bin_index = sizeof(MAX_BIN_COUNT) * 8;
